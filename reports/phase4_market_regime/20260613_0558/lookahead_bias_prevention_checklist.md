...
**Run:** 20260613_0558 · **Framework:** `TREND_STRENGTH_ADX_EMA_SPEC` · **Mode:** SPEC-ONLY.
Independently reviewed by `causal-auditor` → **PASS** (all 6 checks). `llm_discretion_used=false`.

**`passed` values:** `yes` = affirmatively satisfied and verifiable by reading the produced
spec/process; `n/a` = not applicable this run because no labels/data were produced — satisfied by
the classifier/labels **schema design**, not by execution. **No row is `false`/`no`.** Per prompt
§34, only a `false` row blocks completion; an `n/a` row (design-guaranteed, honestly not executed)
does not. No evidence below claims a check was run on real data or labels.
...
| selected framework is theory-grounded | yes | Wilder ADX/DMI + Elder EMA alignment + Mandelbrot/Engle volatility clustering (research §3.1) | reject any LLM-invented regime |
...
| threshold fit train-only (if adaptive) | n/a | thresholds are theory/convention-fixed (ADX=25, P70); the percentile is a causal trailing statistic, not a train-only model fit and not tuned to performance; no fit performed this run | if adaptive thresholds are introduced, fit on train split only, never on test/performance |
...
| causal labels separated from smoothed/posthoc | n/a | no smoothed/posthoc labels produced this run; design mandates separate files (pipeline §11); causal file is the only output | route smoothed/posthoc to their own files with usable_in_hybrid=false |
...
| regime_labels.csv has no strategy-performance column | n/a | no CSV produced (data absent); `regime_labels_schema.md` enumerates and forbids the 14 strategy-performance/future columns; guaranteed by design | drop any forbidden column before writing the CSV |
| llm_discretion_used is false for all labels | n/a | no labels produced; classifier is deterministic (no LLM override); schema fixes `llm_discretion_used=false`; §16.5 fails any true row | invalidate any label with llm_discretion_used=true |

**Result:** 12 rows `yes`, 6 rows `n/a`, **0 rows `false`** → look-ahead checklist passed.
